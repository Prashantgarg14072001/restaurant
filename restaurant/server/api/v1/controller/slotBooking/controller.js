...
const {findID,create,findID2,operation,findID3}= slots;
//********************************************** */
  ...
  async bookSlot(req,res,next){
    ...
    try {
     ...
     if (!user) {
        ...
        const slots = await findID3({_id:id});

        
       
      //  const {slot}=slots;
      const booked= await operation(
      ...
      if(!booked){
        throw apiError.conflict("not booked")
      }else{
        return res.json(new response("booked slot"))
      }
